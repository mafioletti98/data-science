...
# Carregar o Excel, selecionando apenas a coluna específica
df = pd.read_excel(caminho_arquivo, usecols=["capacidade operacional (hectares por hora) (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["capacidade operacional (hectares por hora) (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters nas Notas")
plt.xlabel("Índice dos Dados")
plt.ylabel("Capacidade Operacional (hectares por hora) (csat)")
plt.legend()
plt.tight_layout()
...
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de capacidade operacional.")

# Exibir o gráfico no Streamlit
st.image("grafico_clusters.png")

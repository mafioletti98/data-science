...
df = pd.read_excel(caminho_arquivo, usecols=["facilidade de operação (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["facilidade de operação (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Facilidade de Operação")
plt.xlabel("Índice dos Dados")
plt.ylabel("Facilidade de Operação (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
plt.savefig("grafico_clusters_facilidade.png")

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de facilidade de operação.")

# Exibir o gráfico no Streamlit
st.image("grafico_clusters_facilidade.png")

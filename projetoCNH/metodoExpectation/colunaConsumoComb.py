...
# Carregar o Excel, selecionando a coluna "consumo de combustível (litros por hectares) (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["consumo de combustível (litros por hectares) (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["consumo de combustível (litros por hectares) (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters no Consumo de Combustível (litros por hectares) (csat)")
plt.xlabel("Índice dos Dados")
plt.ylabel("Consumo de Combustível (litros por hectares) (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_consumo_combustivel.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de consumo de combustível (litros por hectares) (csat).")

...
df = pd.read_excel(caminho_arquivo, usecols=["disponibilidade e confiabilidade mecânica  (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["disponibilidade e confiabilidade mecânica  (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Disponibilidade e Confiabilidade Mecânica")
plt.xlabel("Índice dos Dados")
plt.ylabel("Disponibilidade e Confiabilidade Mecânica (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
plt.savefig("grafico_clusters_disponibilidade.png")

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de disponibilidade e confiabilidade mecânica.")

# Exibir o gráfico no Streamlit
st.image("grafico_clusters_disponibilidade.png")

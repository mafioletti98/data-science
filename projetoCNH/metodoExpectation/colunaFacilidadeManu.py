...
# Carregar o Excel, selecionando a coluna "facilidade para realização de manutenções (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["facilidade para realização de manutenções (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["facilidade para realização de manutenções (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Facilidade para Realização de Manutenções (csat)")
plt.xlabel("Índice dos Dados")
plt.ylabel("Facilidade para Realização de Manutenções (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_facilidade_manutencao.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de facilidade para realização de manutenções (csat).")

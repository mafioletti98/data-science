...
# Carregar o Excel, selecionando a coluna "geração e transmissão de dados para gestão da frota (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["geração e transmissão de dados para gestão da frota (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["geração e transmissão de dados para gestão da frota (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Geração e Transmissão de Dados para Gestão da Frota (csat)")
plt.xlabel("Índice dos Dados")
plt.ylabel("Geração e Transmissão de Dados para Gestão da Frota (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_transmissao_dados_frota.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de geração e transmissão de dados para gestão da frota (csat).")

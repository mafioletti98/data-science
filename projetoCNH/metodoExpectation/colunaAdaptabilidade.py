...
# Carregar o Excel, selecionando a coluna "adaptabilidade as mais diversas condições de trabalho (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["adaptabilidade as mais diversas condições de trabalho (csat)"])

# Remover valores ausentes (NaN)
dados_limpos = df.dropna()

# Aplicar o modelo Expectation Maximization (GMM)
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["adaptabilidade as mais diversas condições de trabalho (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Adaptabilidade às Mais Diversas Condições de Trabalho (csat)")
plt.xlabel("Índice dos Dados")
plt.ylabel("Adaptabilidade às Mais Diversas Condições de Trabalho (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_adaptabilidade.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de adaptabilidade às mais diversas condições de trabalho (csat).")

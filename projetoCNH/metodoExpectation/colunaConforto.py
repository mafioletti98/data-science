...
df = pd.read_excel(caminho_arquivo, usecols=["conforto e ergonomia (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["conforto e ergonomia (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters no Conforto e Ergonomia")
plt.xlabel("Índice dos Dados")
plt.ylabel("Conforto e Ergonomia (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
plt.savefig("grafico_clusters_conforto.png")

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de conforto e ergonomia.")

# Exibir o gráfico no Streamlit
st.image("grafico_clusters_conforto.png")

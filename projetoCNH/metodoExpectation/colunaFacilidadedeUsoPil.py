...
# Carregar o Excel, selecionando a coluna "facilidade de uso do piloto automático (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["facilidade de uso do piloto automático (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["facilidade de uso do piloto automático (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters na Facilidade de Uso do Piloto Automático (csat)")
plt.xlabel("Índice dos Dados")
plt.ylabel("Facilidade de Uso do Piloto Automático (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_piloto_automatico.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de facilidade de uso do piloto automático (csat).")

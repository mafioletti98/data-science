...
# Carregar o Excel, selecionando a coluna "custo de manutenção (csat)"
df = pd.read_excel(caminho_arquivo, usecols=["custo de manutenção (csat)"])
...
for cluster in range(3):  # 3 clusters (ajustável)
    ...
    plt.scatter(
        ...
        cluster_data["custo de manutenção (csat)"],  # Eixo Y: valores da coluna selecionada
        label=f"Cluster {cluster}",
        color=cores[cluster]  # Usando as cores definidas
    )

plt.title("Distribuição dos Clusters no Custo de Manutenção")
plt.xlabel("Índice dos Dados")
plt.ylabel("Custo de Manutenção (csat)")
plt.legend()
plt.tight_layout()

# Salvar o gráfico temporariamente
grafico_path = "grafico_clusters_custoManu.png"
plt.savefig(grafico_path)

# Streamlit para visualização
st.title("Análise de Clusters com Expectation Maximization")
st.write("Este é um gráfico mostrando a distribuição dos clusters na coluna de custo de manutenção (csat).")
